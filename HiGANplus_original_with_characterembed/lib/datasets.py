...
from lib.path_config import data_roots, data_paths, ImgHeight, CharWidth, char_data_roots, char_data_paths
from lib.transforms import RandomScale, RandomClip
import torchvision.transforms as transforms
import pickle

class Hdf5Dataset(Dataset):
    def __init__(self, root, split,char_root, char_split, transforms=None, alphabet_key='all', process_style=False, normalize_wid=True):
    ...
    def _load_h5py(self, file_path, char_pickle_file, normalize_wid=True):
        ...
        ### todo: load pickle file for character images
        desired_height, desired_width = 64, 32
        with open(self.char_root + self.char_split, 'rb') as handle:
            self.char_images = pickle.load(handle)
        self.num_char_in_word = []
        """
        for c, char_images in enumerate(self.char_images):
            if self.char_images[c] != 0:

                for ec, each_char_image in enumerate(self.char_images[c]):
                    self.char_images[c][ec] = cv2.resize(self.char_images[c][ec], (32, 64), interpolation=cv2.INTER_AREA)
                if len(self.char_images[c]) < self.num_char:
                    self.num_char_in_word.append(len(self.char_images[c]))
                    self.char_images[c] = self.char_images[c] + [np.zeros((desired_height, desired_width), dtype=np.uint8)] * (self.num_char-len(self.char_images[c]))
                else:
                    self.num_char_in_word.append(self.num_char)
                    self.char_images[c] = self.char_images[c][0:self.num_char]

            else:
                self.num_char_in_word.append(0)
                self.char_images[c] = [np.zeros((desired_height, desired_width), dtype=np.uint8)] * (self.num_char)
        """
        for c, char_images in enumerate(self.char_images):

            if len(self.char_images[c]) < self.num_char:
                self.num_char_in_word.append(len(self.char_images[c]))
                self.char_images[c] = self.char_images[c] + [
                    np.zeros((desired_height, desired_width), dtype=np.uint8)] * (
                                                  self.num_char - len(self.char_images[c]))
            else:
                self.num_char_in_word.append(self.num_char)
                self.char_images[c] = self.char_images[c][0:self.num_char]
    ...
    def __getitem__(self, idx):
        ...
        mean, std = 0.5, 0.5
        char_imgs = self.char_images[idx]
        num_char = self.num_char_in_word[idx]
        # style image
        ...
        #f2_unnormalize = transform(data['style_img'])
        #cv2.imwrite('f1_unnorm_d.png', np.asarray(f2_unnormalize.squeeze(0)) * 255)
        data['org_char_img'] = [torch.squeeze(self.org_transforms(Image.fromarray(deepcopy(c), mode='L')), 0) for c in char_imgs]
        data['org_char_img'] = torch.stack(data['org_char_img'])
        data['num_char_in_word'] = num_char
        """
        cv2.imwrite('o_image.png', 255*data['org_img'].squeeze(0).detach().numpy())
        char_images = data['org_char_img'].detach().numpy()
        for i in range(len(char_images)):
           cv2.imwrite('ch_image' + str(i) + '.png', 255*char_images[i])
        """
        return data
    ...
    def collect_fn(batch):
        org_imgs, org_char_imgs, org_img_lens, style_imgs, style_img_lens, aug_imgs, aug_img_lens,\
        lbs, lb_lens, wids, num_char_in_words = [], [], [], [], [], [], [], [], [], [], []

        for data in batch:
            org_img, org_char_img, style_img, lb, wid, num_char_in_word = data['org_img'], data['org_char_img'], data['style_img'], data['lb'], data['wid'], data['num_char_in_word']
            ...
            org_char_imgs.append(org_char_img)
            num_char_in_words.append(num_char_in_word)
        ...
        bdata['org_char_imgs'] = torch.stack(org_char_imgs).float()
        bdata['num_char_in_words'] = torch.tensor(num_char_in_words).int()
