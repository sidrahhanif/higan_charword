...
from networks.module import Recognizer, WriterIdentifier, StyleEncoder, StyleBackbone, StyleEncoder_w_c_character_style,\
StyleEncoder_char_sep_attention_character_style, StyleEncoder_char_word_attention_style, StyleEncoder_transformer, StyleEncoder_char_word_transformer
...
from networks.loss import recn_l1_loss, CXLoss, KLloss, recn_word_proj_loss

import cv2
from networks.pkl_loss import PKL
#import torch
#import torch.distributed as dist
...
#os.environ["CUDACUDA_VISIBLE_DEVICES"] = "0,1,2"
...
class BaseModel(object):
    ...
    def load(self, ckpt, map_location=None, modules=None):
        ...
        if map_location is None:
            ...
            ckpt = torch.load(ckpt, map_location='cpu') #map_location)
...
class GlobalLocalAdversarialModel(AdversarialModel):
    def __init__(self, opt, log_root='./'):
        ...
        #style_encoder_char = StyleEncoder_char_sep_attention_character_style(**opt.EncModel).to(device)
        style_encoder_char = StyleEncoder_char_word_transformer(**opt.EncModel).to(device)
    ...
    def train(self):
        ...
        #model = self.models().to(device_id)
       # ddp_model = DDP(model, device_ids=[device_id])
        ...
        self.averager_meters = AverageMeterManager(['adv_loss', 'fake_disc_loss',
                                                    ...
                                                    'gp_wid', 'gp_recn']) # 'proj_loss',  ,'gp_proj'
        device = self.device
        ...
        best_fid = np.inf
        iter_count = 0
        #self.models = torch.nn.DataParallel(self.models, device_ids=[1, 2, 3])
        for epoch in range(epoch_done, self.opt.training.epochs):
            for i, batch in enumerate(self.train_loader):
                ...
                real_imgs, real_img_lens, real_wids, real_imgs_char, num_char_in_words  = batch['style_imgs'].to(device), \
                                                      batch['style_img_lens'].to(device), \
                                                      batch['wids'].to(device), batch['org_char_imgs'].to(device),\
                                                      batch['num_char_in_words']  .to(device)
                ...
                set_requires_grad([self.models.G, self.models.E, self.models.E_char, self.models.R, self.models.W, self.models.B], False)
                ...
                with torch.no_grad():
                    ...
                    if self.vae_mode:
                        enc_z, _, _ = self.models.E_char(real_imgs, real_img_lens, self.models.B, vae_mode=True)
                        #print('I am done B')
                    else:
                        enc_z = self.models.E_char(real_imgs, real_img_lens, self.models.B, vae_mode=False)
                    """
                    if self.vae_mode:
                        #print('char')
                        #print(num_char_in_words)
                        enc_z, _, _ = self.models.E_char(real_imgs, real_img_lens,real_imgs_char,num_char_in_words, self.models.B, vae_mode=True)#, vae_mode=True)
                    else:
                        enc_z = self.models.E_char(real_imgs, real_img_lens, real_imgs_char, num_char_in_words, self.models.B,  vae_mode=False) #, vae_mod
                    """
                    style_imgs = self.models.G(enc_z, fake_lbs, fake_lb_lens)
                    recn_imgs = self.models.G(enc_z, real_lbs, real_lb_lens)
                ...
                if iter_count % self.opt.training.num_critic_train == 0:
                    ...
                    set_requires_grad([self.models.G, self.models.E, self.models.E_char], True)
                    # self.models.B.frozen_bn()
                    ...
                    """
                    if self.vae_mode:
                        (enc_z, mu, logvar), real_img_feats = self.models.E_char(real_imgs, real_img_lens, real_imgs_char, num_char_in_words, self.models.B,
                                                                            ret_feats=True, vae_mode= True)
                    else:
                        enc_z = self.models.E_char(real_imgs, real_img_lens, real_imgs_char, num_char_in_words,  self.models.B,
                                                              ret_feats=True, vae_mode=False)
                    """
                    style_imgs = self.models.G(enc_z, fake_lbs, fake_lb_lens)
                    recn_imgs = self.models.G(enc_z, real_lbs, real_lb_lens)
                    #print('recn:', recn_imgs.shape)
                    ### todo: remove the padded part from reconstructed image



                    ### todo: divide the reconstructed image into equally spaced windows and crop them



                    ### todo: resize the cropped pathces into deminsions 64x32
                    ...
                    real_imgs = torch.tensor(real_imgs , requires_grad=True)
                    #print('recn_loss: ', recn_imgs)
                    #print('real image: ', real_imgs)
                    #projected_loss = PKL(recn_imgs, real_imgs, real_img_lens, 127)
                    #print('projected loss:', projected_loss)
                    #exit(1)
                    ...
                    #grad_fake_proj = torch.autograd.grad(projected_loss, enc_z, create_graph=True, retain_graph=True)[0]
                    ...
                    #gp_proj = torch.div(std_grad_adv, torch.std(grad_fake_proj) + 1e-8).detach() + 1
                    ...
                    #self.averager_meters.update('gp_proj', gp_proj.item())
                    ...
                    #self.averager_meters.update('proj_loss', projected_loss.item())
                ...
                if iter_count % self.opt.training.print_iter_val == 0:
                    ...
                    #print(meter_vals['recn_loss'], meter_vals['proj_loss'])
                    info = "[%3d|%3d]-[%4d|%4d] G:%.4f G-p:%.4f D-fake:%.4f D-real:%.4f " \
                           ...
                           % (epoch, self.opt.training.epochs,
                              ...
                              meter_vals['ctx_loss'], meter_vals['kl_loss']) #,, meter_vals['proj_loss'] proj-z:%.4f
